...
// Serve static files from ROOT (HTML files are now directly in root)
...
// ====================== ROUTES ======================
// All API routes MUST come BEFORE the catch-all route
...
app.post('/register', async (req, res) => {
    ...
    try {
        ...
        const user = new User({ email, password: hashed, role: "member" });
        await user.save();
        const token = jwt.sign({ email: user.email, role: user.role }, JWT_SECRET, { expiresIn: '24h' });
        res.json({ success: true, token, email: user.email, role: user.role });
    } catch (e) {
...
// Serve login.html for any unknown route
app.get('*', (req, res) => {
